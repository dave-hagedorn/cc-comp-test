...
def _make_includes_rel_to_rundir(ctx, command_line):
    ...
    #
    # So, all include paths generated by the cc_* rules are something like bazel-out/k8-fastbuild/bin/ (or whatever target/arch)
    # The rest of the include path then matches the paths of the headers in the workspace folder - say "lib/comp_test"
    #
    # Side-note - for cc_library's that specify include_prefix, includes go under, say "lib/comp_test/_virtual_includes/<include_prefix>/
    ...
    #  lib/comp_test/_virtual_includes/<include_prefi>/...
...
def cc_comp_test(name, src = None, copts = [], deps = []):
    ...
    Just like cc_test() but for testing compile time behaviour like static_assert().

    Use this to write test cases for APIs in your library that should static_assert() under expected conditions,
    or that should compile (not status_assert()) under others, etc.
