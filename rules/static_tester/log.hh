...
template <std::size_t N> constexpr auto is_string_literal(const char (&)[N]) {
    return true;
}
...
template <typename T> struct inner_type { using INNER = void; };
...
template <typename T> fmt::color color_for() {
